...
RSpec.describe "destroy telescope" do
  it 'can delete telescope and related images from the database' do
    ...
    expect(page).to_not have_content(@pia12108.name)
    expect(page).to_not have_content(@nasty_17754652960_o.name)

    visit "/telescopes"

    expect(page).to_not have_content(@hubble.name)
  end
